...
✅ وضعیت: در registry عمومی (core/gateways/saman.py) — پس از **تست تراکنش واقعی
با ترمینال واقعی** منتقل شد (نسخه‌ی 1.0.0). get_gateway("saman") مستقیم کار می‌کند.
سامان sandbox واقعی ندارد و فقط live است (sandbox=True خطا می‌دهد). نیاز به
TerminalId واقعی و ثبت IP سرور نزد سامان.
    ...
    IRANIAN_PAYMENT = {
        ...
        "gateways": {
            "saman": {
                ...
                # ⛔ سامان sandbox ندارد: "sandbox": True (اینجا یا ارث از سراسری)
                # باعث GatewayConfigurationError می‌شود و برنامه اجرا نمی‌شود. سامان
                # فقط live است. اگر sandbox سراسری True است، اینجا "sandbox": False بگذار.
            },
        },
    }

    # sandbox مجزای هر درگاه: برای درگاه‌هایی که URL سندباکس جدا دارند
    # (زرین‌پال/دیجی‌پی) کلید "sandbox" داخل config درگاه بر مقدار سراسری اولویت دارد.
    # ⛔ سامان و ملت sandbox ندارند و "sandbox": True را رد می‌کنند (خطا).
...
def callback_self_managed(request):
    ...
    extra = {
        k: v
        for k, v in {"ref_num": p.get("RefNum"), "state": p.get("State")}.items()
        if v
    }
