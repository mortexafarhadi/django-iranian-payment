"""
درگاه پی‌پینگ (payping) — REST — تجربی و تست‌نشده.
...
در registry عمومی نیست. فقط با import صریح در دسترس است:
    from django_iranian_payment.experimental import PayPingGateway

نکات این درگاه: فین‌تک REST/JSON با Bearer token. ⚠️ پی‌پینگ مبلغ را به تومان می‌گیرد — تبدیل ریال÷۱۰ داخل درگاه. callback GET.
...
class PayPingGateway(BaseGateway):
    ...
    def initiate(self, request: PaymentRequest) -> InitiateResult:
        # TODO: payload را طبق مستندات پی‌پینگ بساز.
