"""
درگاه نکست‌پی (nextpay) — REST — تجربی و تست‌نشده.
...
در registry عمومی نیست. فقط با import صریح در دسترس است:
    from django_iranian_payment.experimental import NextPayGateway

نکات این درگاه: فین‌تک REST/JSON. callback از نوع GET. مبلغ به ریال.
...
class NextPayGateway(BaseGateway):
    ...
    def initiate(self, request: PaymentRequest) -> InitiateResult:
        # TODO: payload را طبق مستندات نکست‌پی بساز.
