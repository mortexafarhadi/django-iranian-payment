"""
درگاه پرداخت الکترونیک صادرات (saderat) — REST — تجربی و تست‌نشده.
...
در registry عمومی نیست. فقط با import صریح در دسترس است:
    from django_iranian_payment.experimental import SaderatGateway

نکات این درگاه: callback از نوع GET. مستندات: درگاه صادرات (سپ/به‌پرداخت صادرات).
...
class SaderatGateway(BaseGateway):
    ...
    def initiate(self, request: PaymentRequest) -> InitiateResult:
        # TODO: payload را طبق مستندات پرداخت الکترونیک صادرات بساز.
