"""
درگاه پاسارگاد (pasargad) — REST — تجربی و تست‌نشده.
...
در registry عمومی نیست. فقط با import صریح در دسترس است:
    from django_iranian_payment.experimental import PasargadGateway

نکات این درگاه: verify دومرحله‌ای (check سپس verify). امضای RSA لازم دارد — هنگام تست با مستندات PEP پر شود.
...
class PasargadGateway(BaseGateway):
    slug = "pasargad"
    requires = ('username', 'password', 'terminal_number', 'merchant_code')
    ...
    def initiate(self, request: PaymentRequest) -> InitiateResult:
        # TODO: payload را طبق مستندات پاسارگاد بساز.
